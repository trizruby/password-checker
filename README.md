# password-checker
Verificador de força de senha desenvolvido em Python para avaliar a segurança das senhas.

Este projeto é um verificador de força de senha desenvolvido em Python. Ele analisa uma senha e determina se ela é fraca, média ou forte com base no tamanho e na variedade de caracteres.

📌Funcionalidades

-Verifica se uma senha é fraca ou forte

-Analisa o tamanho da senha

-Detecta números, letras maiúsculas e símbolos

-ornece um feedback sobre a segurança da senha

📌Como executar:

Exemplo:

1.Clone este repositório

2.Abra a pasta do projeto

3.Execute o arquivo Python

📌Exemplo

Digite sua senha: MyPassword123!

Força da senha: Forte

--------------------------------------------------------------------------------------------------

Este projeto foi criado para praticar programação em Python e conceitos básicos de cibersegurança.
